...
function ensureTestApisIntercepted() {
    if (!testApisIntercepted) {
        ...
        if (process.env.NEXT_PRIVATE_TEST_PROXY === "true") {
            const { interceptTestApis, wrapRequestHandler } = __webpack_require__(87);
            interceptTestApis();
...
async function middleware(request) {
    ...
    if (pathname.startsWith("/admin") || pathname === "/maintenance" || pathname.startsWith("/api") || pathname.startsWith("/_next") || pathname.includes(".") || pathname.startsWith("/favicon")) {
